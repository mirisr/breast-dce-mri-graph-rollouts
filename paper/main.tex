...
The ablation design separates three questions. The first question is whether the
original scheduled-sampling graph rollout is correctly centered in FTV. This is
tested by comparing the original graph rollout against \calibrationtag, which
keeps the same graph-forecasting family but adds endpoint and active-burden
losses. The second question is whether graph neighborhoods add information once
the endpoint is calibrated. This is tested by no-edge, spatial, radial,
feature-only, radial imaging-feature, and hybrid spatial-feature edge variants
trained under matched endpoint-calibrated settings. The third question is
whether the graph is necessary when only scalar final-visit FTV is requested.
This is tested with last-observed, temporal ridge, kernel temporal, and hybrid
graph-plus-scalar centers using the same residual-MC family.

\begin{table}[t]
\centering
\caption{Comparator roles used to define the graph-state claim.}
\label{tab:comparatorroles}
\scriptsize
\setlength{\tabcolsep}{3.0pt}
\begin{tabularx}{\columnwidth}{@{}lX@{}}
\toprule
Comparator & Role in the evaluation \\
\midrule
Original graph rollout & Tests the uncalibrated spatial rollout center. \\
\calibrationtag{} & Tests whether endpoint and active-burden losses correct the graph readout. \\
No-edge endpoint graph & Tests node-local forecasting without message passing. \\
Radial/Hybrid edge graphs & Tests whether neighborhood structure improves the calibrated graph state. \\
Scalar residual MC & Tests the best scalar-only FTV readout boundary. \\
Hybrid graph+scalar MC & Tests whether graph and scalar centers are complementary for scalar FTV. \\
\bottomrule
\end{tabularx}
\end{table}

These comparators are intentionally not all expected to favor the graph model.
If the target is only scalar FTV, a scalar residual model can be sufficient or
better. The retained graph model is therefore judged on the joint requirement
of producing a calibrated FTV distribution while preserving a future tumor
graph whose active-node burden and sampled-cloud geometry can be evaluated.
...
plausible while miscentered in FTV. The evaluation therefore treats endpoint
accuracy, distribution calibration, and graph-state plausibility as distinct
claims.
...
study diagnostic rather than decorative: a gain in the latent-field regime
supports the premise that local neighborhoods can denoise hidden regional
response structure, while a weak gain in the node-local regime prevents the
paper from claiming that edges are universally helpful. The real-cohort edge
ablations are interpreted through this same lens.
...
The endpoint-calibration result is also a methodological caution. A future
tumor graph can look plausible in sampled-cloud metrics while still producing a
biased total active burden. Conversely, a scalar FTV center can be accurate while
discarding the local tumor state entirely. Treating FTV as a graph readout
rather than an independent regression head keeps these two targets connected:
the endpoint loss corrects the clinical burden center, while the active-burden
term keeps the correction attached to the predicted node activity. This is the
reason the retained model is evaluated on active-node error, SWD, Chamfer, Dice,
coverage, width, and CRPS rather than on FTV MAE alone.
...
for recalibration before deployment. That failure mode is useful because it
points to the next validation requirement rather than hiding transfer risk
